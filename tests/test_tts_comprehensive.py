...
class TestBasicTTS:
    ...
    def test_explicit_language_specification(self, engine):
        ...
        print(f"English synthesis audio duration: {len(en_audio) / engine.sample_rate:.2f}s")
        ...
        print(f"Chinese synthesis audio duration: {len(zh_audio) / engine.sample_rate:.2f}s")
...
class TestStreamingTTS:
    ...
    def test_english_streaming_tts(self, engine):
        ...
        for audio in engine.tts_stream(english_stream_text.strip(), language='en'):
            ...
            assert len(audio) > 0, f"English segment {english_segments} should not be empty"
    ...
    def test_chinese_streaming_tts(self, engine):
        ...
        for audio in engine.tts_stream(chinese_stream_text.strip(), language='zh'):
            ...
            assert len(audio) > 0, f"Chinese segment {chinese_segments} should not be empty"
...
class TestStreamingInput:
    ...
    def test_streaming_input_comprehensive(self, engine):
        ...
        en_segments = _test_feed_streaming(engine, english_conv_text, "English Conversational")
        ...
        zh_segments = _test_feed_streaming(engine, chinese_tech_text, "Chinese Technical")
...
def _test_feed_streaming(engine, text, test_name):
    ...
    def token_producer():
        print(f"  📝 Feeding {len(tokens)} tokens for {test_name} (language: {language})...")
    ...
    def audio_consumer():
        for audio in engine.feed_out():
            ...
            duration = len(audio) / engine.sample_rate
            print(f"  🔊 {test_name} segment {len(segments_received)}: {audio.shape} ({duration:.2f}s)")
    ...
    print(f"  ✅ {test_name}: {len(segments_received)} segments, {total_audio:.1f}s total audio")
...
class TestSegmentationEquivalence:
    ...
    def test_segmentation_equivalence(self, engine):
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            engine.reset()
            expected_segments = engine.segmenter.segment_sentences(test_case['text'], language=test_case['language'])
            ...
            segmentation_result = analyze_feed_segmentation(engine, test_case['text'], test_case['language'])
            
            # Compare segmentation results
            print(f"  🔍 Segmentation comparison:")
            ...
            print(f"    Buffer final segments: {len(segmentation_result['final_segments'])}")
            
            # Assertions for segmentation equivalence
            assert ref_segment_count == len(expected_segments), f"tts_stream segments should match expected"
            # Note: processed_segments may be different due to streaming buffer timing, 
            # but final segmentation should be equivalent
            assert len(segmentation_result['final_segments']) == len(expected_segments), f"Final segments should match expected"
            ...
            assert original_no_space == reconstructed_no_space, f"Content should match (ignoring whitespace): expected='{original_no_space}', got='{reconstructed_no_space}'"
...
class TestResetFunctionality:
    ...
    def test_reset_functionality(self, engine):
        ...
        restart_text = "Clean restart test after multiple resets. This should work perfectly."
        restart_segments = _test_feed_streaming(engine, restart_text, "Multi-Reset Recovery")
...
class TestEdgeCases:
    ...
    def test_available_languages_and_voices(self, engine):
        ...
        assert 'en' in languages and 'zh' in languages, "Should support English and Chinese"
        
        en_voices = engine.get_voices('en')
        zh_voices = engine.get_voices('zh')
        assert len(en_voices) > 0 and len(zh_voices) > 0, "Should have voices for each language"
    ...
    def test_mixed_scripts_with_explicit_language(self, engine):
        ...
        assert len(mixed_audio_en) > 0, "Mixed scripts should work with English language"
        
        mixed_scripts_zh = "Hello 你好 world 世界！"
        mixed_audio_zh = engine.tts(mixed_scripts_zh, language='zh')
        assert len(mixed_audio_zh) > 0, "Mixed scripts should work with Chinese language"

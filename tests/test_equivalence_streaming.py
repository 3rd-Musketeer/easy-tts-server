...
def compare_segmentation_modes(engine, text, test_name):
    ...
    print(f"  🔄 Comparing segmentation for: {text[:50]}{'...' if len(text) > 50 else ''}")
    ...
    success = compare_segmentation_results(reference_result, streaming_result, test_name)
...
def analyze_stream_segmentation(engine, text) -> Dict:
    ...
    for i, segment in enumerate(segments):
        ...
        print(f"    📊 Segment {i+1}: '{segment}' ({len(segment)} chars, {len(segment.split())} words)")
    ...
    def token_producer():
        ...
        for i, token in enumerate(tokens):
            ...
            if processed_after > processed_before:
                ...
                if ENABLE_DETAILED_DEBUG:
                    print(f"      🎯 Triggered {processed_after - processed_before} segment(s)")
...
def compare_segmentation_results(reference_result, streaming_result, test_name) -> bool:
    ...
    if ref_count != actual_segments_count:
        print(f"    ❌ Final segment count mismatch: {ref_count} vs {actual_segments_count}")
        return False
    
    print(f"    ✅ Segment counts match: {ref_count}")
    
    # 2. Check segment text equivalence
    print(f"    📝 Segment text comparison:")
    segments_match = True
    
    for i, (ref_seg, stream_seg) in enumerate(zip(ref_segments, stream_segments)):
    ...
    if reference_result['original_text'].strip() != stream_reconstructed.strip():
        print(f"    ❌ Text reconstruction differs:")
        print(f"      Original: '{reference_result['original_text']}'")
        print(f"      Reconstructed: '{stream_reconstructed}'")
        return False
    
    print(f"    ✅ Text reconstruction matches")
    
    # 4. Check segmentation timing (if available)
    events = streaming_result['segmentation_events']
    trigger_events = [e for e in events if e.get('type') in ['token_triggered', 'flush_triggered']]
    
    print(f"    ⏱️  Segmentation timing analysis:")
    print(f"      Total trigger events: {len(trigger_events)}")
    
    for event in trigger_events:
        if event['type'] == 'token_triggered':
            print(f"        Token {event['token_index']+1} ('{event['token']}'): triggered {event['segments_triggered']} segment(s)")
    ...
    if total_triggered != ref_count:
        print(f"    ⚠️  Total triggered segments ({total_triggered}) != expected ({ref_count})")
        # This is a warning, not a failure, as timing can vary
    
    print(f"    ✅ Segmentation equivalence verified")
    return True
...
def main():
    ...
    try:
        ...
        test_cases = [
            ...
            ("Complex punctuation", "Question: What's the weather like? Answer: It's sunny! Note: Bring an umbrella; just in case."),
            ("Long conversational", "Hello there! How are you doing today? I hope you're having a wonderful time. Let me tell you a story about artificial intelligence."),
            ("Technical content", "The TTS engine implements a modular architecture. Core components include synthesis, streaming, and task management."),
            ("Mixed content", "Welcome to our service! Please note: processing may take 2-3 minutes. Status: ready. Would you like to continue?"),
        ]

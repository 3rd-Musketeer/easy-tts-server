...
class SynthesisCore:
    ...
    def _load_models(self):
        ...
        self.pipelines = {
            'en': {
                'american': KPipeline(lang_code='a', repo_id=self.repo_id, model=self.model),
                'british': KPipeline(lang_code='b', repo_id=self.repo_id, model=self.model)
            },
            'zh': KPipeline(lang_code='z', repo_id=self.repo_id, model=self.model, en_callable=self._en_callable)
        }
    ...
    def synthesize(self, text: str, language: str, voice: str) -> np.ndarray:
        ...
        if not language or language not in self.voices:
            language = 'en'
            logger.warning(f"Invalid language, falling back to English")

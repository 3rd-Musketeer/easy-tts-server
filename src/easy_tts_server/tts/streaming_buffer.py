...
class StreamingBuffer:
    ...
    def feed_token(self, token: str, language: str):
        ...
        if language not in self.segmenter.get_available_languages():
            raise ValueError(f"Unsupported language: {language}. Supported languages: {self.segmenter.get_available_languages()}")
        ...
        if self.current_language is None:
            ...
            logger.warning(f"Language changed from {self.current_language} to {language}. Resetting buffer.")
            self.reset()
            self.current_language = language
    ...
    def flush(self) -> int:
        ...
        current_segments = self.segmenter.segment_sentences(cleaned_buffer, language=self.current_language)
        
        if len(current_segments) > self.processed_segments:
            remaining = current_segments[self.processed_segments:]
            voice = self.task_manager.synthesis_core.get_default_voice(self.current_language)
    ...
    def _process_segments(self):
        ...
        if len(current_segments) > self.processed_segments:
            ...
            if segments_to_process:
                voice = self.task_manager.synthesis_core.get_default_voice(self.current_language)
                self.task_manager.queue_tasks(segments_to_process, self.current_language, voice)

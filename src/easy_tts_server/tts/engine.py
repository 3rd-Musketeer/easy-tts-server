...
class TTSEngine:
    ...
    def tts(self, text: str, language: str, voice: str = None) -> np.ndarray:
        """
        ...
        if language not in self.synthesis_core.get_available_languages():
            raise ValueError(f"Unsupported language: {language}. Supported languages: {self.synthesis_core.get_available_languages()}")
        ...
        audio_segments = [self.synthesis_core.synthesize(seg, language, voice) for seg in segments]
        
        return np.concatenate(audio_segments) if audio_segments else np.array([], dtype=np.float32)
    
    def tts_stream(self, text: str, language: str, voice: str = None) -> Generator[np.ndarray, None, None]:
        """
    ...
    def feed_in(self, token: str, language: str):
        """
